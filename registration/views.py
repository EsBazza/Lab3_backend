...
from django.urls import reverse  # Added missing import
...
def student_list(request):
    if request.method == 'GET':
        students = UserRegistration.objects.all()  # Assuming Student is same as UserRegistration for now
...
def student_detail(request, pk):
    try:
        student = UserRegistration.objects.get(pk=pk)  # Assuming same model
    except UserRegistration.DoesNotExist:
    ...
    if request.method == 'GET':
        ...
        serializer = RegistrationSerializer(student, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
